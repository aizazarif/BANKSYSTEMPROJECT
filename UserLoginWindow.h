...
private:
    ...
    Button backButton;
public:
    UserLoginWindow(sf::RenderWindow& win, BankSystem& bs, User* usr)
        : window(win), bankSystem(bs), user(usr),
        loginButton("Login", { 160, 50 }, 20, sf::Color(255, 0, 0), sf::Color::White),
        signupButton("Sign Up", { 160, 50 }, 20, sf::Color(255, 0, 0), sf::Color::White),
        resetButton("Reset", { 160, 50 }, 20, sf::Color(255, 0, 0), sf::Color::White),
        accountField({ 410, 40 }, font, "@USERNAME", false),
            ...
            while (window.pollEvent(event)) {
                ...
                if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left) {
                    if (loginButton.buttonClicked(window)) {
                        ...
                        bool validity = bankSystem.authenticateUserFromFile(username,password);
                        if (validity) {
                            // Open UserDashboardWindow
                            UserDashboardWindow dashboardWindow(window, bankSystem,user);
                            dashboardWindow.run();
                        }
                        else {
                        ...
                        UserSignUpWindow signUpWindow(window, bankSystem);
                        signUpWindow.run();
                    }
